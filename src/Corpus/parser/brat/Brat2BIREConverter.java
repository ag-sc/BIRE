...
public class Brat2BIREConverter {
	 ...
	 * @throws IOException
	 */
	public static List<AnnotatedDocument> convert(BratAnnotatedDocument bratDoc, Corpus corpus,
		...
		Log.d("Split BratDocument %s in %s AnnotatedDocuments", bratDoc.getTextFilename(), tokenizations.size());
	...
	private static boolean isInSentence(BratTextBoundAnnotation ann, Tokenization tokenization) {
		return ann.getStart() >= tokenization.absoluteStartOffset && ann.getEnd() <= tokenization.absoluteEndOffset;
	}
	...
	private static void convertTextBoundAnnotation(State state, AnnotationConfig config, Tokenization tokenization,
		...
		Log.d("---- Annotation: %s (%d-%d) ----", t.getText(), t.getStart(), t.getEnd());
	...
	private static void convertEventAnnotation(State state, AnnotationConfig config, Tokenization tokenization,
		...
		int fromTokenIndex = findTokenForPosition(e.getTrigger().getStart(), tokenization);
		int toTokenIndex = findTokenForPosition(e.getTrigger().getEnd() - 1, tokenization);
		Log.d("---- Annotation: %s (%d-%d) ----", e.getTrigger().getText(), e.getTrigger().getStart(),
				e.getTrigger().getEnd());
	...
	private static int findTokenForPosition(int documentLevelCharacterPosition, Tokenization tokenization) {
		...
		return ParsingUtils.binarySearch(sentenceLevelCharacterPosition, tokenization.tokens, true);
	}
